...
void* s21_trim(const char* src, const char* trim_chars) {
  ...
  if (src && trim_chars) {
    ...
    // передвигаем указатель начала, пока находим в начале src символ из
    // trim_chars
    for (; *start && s21_strchr(trim_chars, *start); start++)
      ;

    // передвигаем указатель конца, пока находим в конце src символ из
    // trim_chars
    for (; end != start && s21_strchr(trim_chars, *(end - 1)); end--)
      ;

    // выделим необходимое кол-во памяти для строки результата
    // (длину определим найдя разниц между указателями границ  + 1 для символа
    // конца строки)
